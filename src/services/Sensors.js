...
import { fetch_sensoritem_data, fetch_iotdevicesensor_data, fetch_locationsensor_data, fetch_manufacturers_data, fetch_sensortype_data, fetch_parameter_data } from '../actions/sensoritem';
...
export function fetchIOTDeviceData(id) {
   return dispatch => new Promise(async (resolve, reject) => {
      ...
      try {
         ...
         axios.get(`${SERVER_URL}location/list?entity=${id}`, { headers: HEADER })
            .then(async (res) => {
               statusMessage(dispatch, "loading", false);
               resolve(
                  dispatch(fetch_locationsensor_data(res.data.list))
               );
            });
      } catch (error) {
         reject(error);
